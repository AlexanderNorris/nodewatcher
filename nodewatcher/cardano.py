...
LAST_BYRON_BLOCKS = [
    ...
    [
        359,
        bytes.fromhex(
            "9c0fe75b6a0499e9576a09589a5777e7021824e8a6d037065829423f861a9bb6"
        ),
    ],
]
...
class Node:
    ...
    def mode_bit_manipulation(self, protocol_id: int, mode: int):
        ...
        mode_mini_protocol_binary = bitstring.BitArray(
            bin=mode_binary + protocol_binary
        )
    ...
    def add_headers(self, obj, protocol_id: int, mode: int = 0):
        ...
        header["protocol_id"] = str(self.convert_bits(protocol_id.to_bytes(2, "big")))[
            :15
        ]  # unusued but available for reference
        # # Mode Mini protocol
        ...
        logging.debug(
            "Request Mode Binary: " + self.convert_bits(header["mode_mini_protocol_id"])
        )
        logging.debug("Length: " + self.convert_bits(header["length"]))
        msg = (
            header["time"]
            + header["mode_mini_protocol_id"]
            + header["length"]
            + cbor_obj
        )
        return msg
    ...
    def handshake(self):
        ...
        obj = [
            0,
            {
                ...
                2: network_magic,
                3: network_magic,
                4: [network_magic, False],
                5: [network_magic, False],
                6: [network_magic, False],
                7: [network_magic, False],
                8: [network_magic, False],
            },
        ]
        protocol_id = 0
